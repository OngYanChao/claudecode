...
// Helper to create UIMessage with parts
...
test("MessageList renders user messages", () => {
  const messages: UIMessage[] = [
    createMessage("1", "user", "Create a button component"),
  ];
...
test("MessageList renders assistant messages", () => {
  const messages: UIMessage[] = [
    createMessage("1", "assistant", "I'll help you create a button component."),
  ];
...
test("MessageList shows content for assistant message with text part", () => {
  const messages: UIMessage[] = [
    createMessage("1", "assistant", "Generating your component..."),
  ];

  render(<MessageList messages={messages} isLoading={true} />);

  // When there's text content, it should be shown
  expect(screen.getByText("Generating your component...")).toBeDefined();
  // The loading indicator is shown along with content during streaming
...
test("MessageList doesn't show loading state for non-last messages", () => {
  const messages: UIMessage[] = [
    ...
    createMessage("2", "user", "Another request"),
  ];
...
test("MessageList renders multiple messages in correct order", () => {
  const messages: UIMessage[] = [
    createMessage("1", "user", "First user message"),
    createMessage("2", "assistant", "First assistant response"),
    createMessage("3", "user", "Second user message"),
    createMessage("4", "assistant", "Second assistant response"),
  ];
...
test("MessageList handles empty parts with text", () => {
  const messages: UIMessage[] = [
    {
      id: "1",
      role: "assistant",
